...
def barplots(charge_counts, charge_counts_by_offense):
    ...
    sns.barplot(data=charge_counts_by_offense, 
                x='charge_degree',y='count',hue='offense_category')
...
def histograms(pred_universe):
    ...
    sns.histplot(data=pred_universe, 
                 x='prediction_nonfelony', stat='probability',bins=[0, .25, .8, 1])
...
def scatterplot(pred_universe):
    ...
    sns.lmplot(data=pred_universe, 
               x='prediction_felony', y='prediction_nonfelony',fit_reg=False)
    ...
    sp = sns.lmplot(data=pred_universe, 
                    x='prediction_felony', y='prediction_nonfelony', hue='race')
    sp.ax.axline(xy1=(0, 0), xy2=(1, 1), color='b', dashes=(2, 2))
    plt.savefig('./data/part2_plots/scatterplot4.png', bbox_inches='tight')

    sp = sns.lmplot(data=pred_universe, 
                    x='prediction_felony', y='prediction_nonfelony', hue='race', col='sex')
